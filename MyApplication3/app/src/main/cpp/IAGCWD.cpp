...
void downscaleIAGCWD(const cv::Mat & src, cv::Mat & dst)
{
    __android_log_print(ANDROID_LOG_ERROR, "TRACKERS", " [IMG_PROC] IAGCWD Downscale src row : %d cols : %d", src.rows,src.cols);
    cv::resize(src, dst, cv::Size(src.rows/2,src.cols/2));
    __android_log_print(ANDROID_LOG_ERROR, "TRACKERS", " [IMG_PROC] IAGCWD Downscale dst row : %d cols : %d", dst.rows,dst.cols);
}

void upscaleIAGCWD(const cv::Mat & src, cv::Mat & dst)
{
    __android_log_print(ANDROID_LOG_ERROR, "TRACKERS", " [IMG_PROC] IAGCWD Upscale src row : %d cols : %d", src.rows,src.cols);
    cv::resize(src, dst, cv::Size(src.rows*2,src.cols*2));
    __android_log_print(ANDROID_LOG_ERROR, "TRACKERS", " [IMG_PROC] IAGCWD Upscale dst row : %d cols : %d", dst.rows,dst.cols);
}